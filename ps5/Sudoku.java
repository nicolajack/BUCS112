...
public class Sudoku {    
    ...
    // a field to keep track of whether or not a certain number is already in a certain column or row (needs to be fixed)
    ...
    public Sudoku() {
        ...
        this.isInRow = new boolean[9][10];
        this.isInCol = new boolean[9][10];
    }
    ...
    // a helper method to determine if a value can be placed in a specific position
    private boolean isSafe(int val, int row, int col){
        if (!this.valIsFixed[row][col]){
            if (!this.subgridHasVal[row/3][col/3][val]){
                if (!this.isInRow[row][val]){
                    if (!this.isInCol[col][val]){
                        return true;
                    }
                }
            }
    ...
    private boolean solveRB(int n) {
        ...
        // to check if an element can be placed or if the valuse is fixed
        if (this.valIsFixed[row][col]){
